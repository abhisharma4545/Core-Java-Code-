
import java.io.IOException;
import java.sql.SQLException;

public class E3 {
          public static void main(String[] args) throws IOException,ArithmeticException{
	                   Hello h = new Hai();
			                h.m1(1);

			}

		}
		class AException extends IOException{}

		class Hello{
			void m1(int x)throws IOException,ArithmeticException{
				if(x==1) {
					throw new ArithmeticException();
				}
				else {
					throw new IOException();
				}
		}
		} 
		class Hai extends Hello{
//			public void m1(int x) {
//				System.out.println("Hai-m1");
//			}
			public void m1(int x)throws IOException,ArithmeticException{
				System.out.println("Hai-m2");
			}
			public void m3(int x)throws AException,ArithmeticException{
				System.out.println("Hai-m3");
			}
			public void m4(int x)throws Exception,ArithmeticException{
				System.out.println("Hai=m4");
			}
//			not ok
			public void m5(int x)throws AException,ArithmeticException,SQLException{
				System.out.println("Hai-m5");
			}
			
		

	}


